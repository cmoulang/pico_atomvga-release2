...
#define WRITE_ONLY 0b00
#define READ_WRITE 0b01
#define NO_ACCESS 0b10
#define READ_ONLY 0b11
...
static void eb_read_init(PIO pio, uint sm, int delay, int permission_address)
{
    uint offset;

    // Overwrite the delay instruction
    uint16_t x = eb_read_program_instructions[eb_read_offset_set_delay];
    x = (x & 0xffe0) | (delay & 0x1f);
    ...
    pio_sm_put(pio, sm, permission_address >> ADD_BITS);
    pio_sm_exec(pio, sm, pio_encode_pull(false, true));
    pio_sm_exec(pio, sm, pio_encode_mov(pio_x, pio_osr));
}

static void eb_address_init(PIO pio, int sm, int memory_address)
{
    int offset;
    ...
    pio_sm_put(pio, sm, memory_address >> ADD_BITS);
    pio_sm_exec(pio, sm, pio_encode_pull(false, true));
    pio_sm_exec(pio, sm, pio_encode_mov(pio_x, pio_osr));
}
...
uint perm_address_chan;
uint perm_data_chan;
uint mem_address_chan;
uint mem_address_chan2;
uint mem_rdata_chan;
uint mem_wdata_chan;
uint last_address_chan;
volatile uint last_address;
                         ...
                         int eb_write_sm)
{
    ...
    last_address_chan = dma_claim_unused_channel(true);
    ...
    // Copies address from mem_rdata_chan to last_address
    // TODO - make the last_address information usable by c-code
    c = dma_channel_get_default_config(last_address_chan);
    channel_config_set_high_priority(&c, true);
    ...
    channel_config_set_read_increment(&c, false);
    channel_config_set_write_increment(&c, false);
    channel_config_set_chain_to(&c, last_address_chan);
    dma_channel_configure(
        mem_wdata_chan,
        &c,
...
{
    uint eb_read_sm = 0;
    eb_read_init(pio, eb_read_sm, 5, (uint)&permission);
